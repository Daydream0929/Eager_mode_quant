            ...
            if type(m) == InvertedResidual:
                for idx in range(len(m.conv)):
                    if type(m.conv[idx]) == nn.Conv2d:
                        fuse_modules(m.conv, [str(idx), std(idx + 1)], inplace=True)
...
# Calibration with the training set
evaluate(myModel, criterion, data_loader, neval_batches=num_calibration_batches)
print('Post Training Quantization: Calibration done')

# Convert to quantized mode
...
This is because we used a simple min/max observer to determine quantization parameters.
Nevertheless, we did reduce the size of our model down to just under 3.6 MB, almost a 4x decrease.
'''
...
print('Evaluation accuracy on %d images, %2.2f'%(num_eval_batches * eval_batch_size, top1.avg))
torch.jit.save(torch.jit.script(per_channel_quantized_model), saved_model_dir + scripted_float_model_file)
...
def run_benchmark(model_file, img_loader):
    ...
    for i, (image, target) in enumerate(img_loader):
        if i < num_batches:
            start = time.time()
            output = model(image)
            end = time.time()
            ...
            break
    num_images = image.size()[0] * num_batches
